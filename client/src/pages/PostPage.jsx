                ...
                <div className="mt-12">
                    ...
                    <div className="space-y-4">
                        {post.comments.map(comment => (
                            <div key={comment._id} className="p-4 rounded-lg shadow-sm">
                                <p className="font-semibold">{comment.user}</p>
                                <p className="text-xs text-gray-500 mb-2">{new Date(comment.createdAt).toLocaleString()}</p>
                                {editingComment.id === comment._id ? (
                                    <div>
                                        <textarea ref={editCommentTextareaRef} value={editingComment.content} onChange={(e) => setEditingComment({ ...editingComment, content: e.target.value })} className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md dark:bg-gray-700 focus:ring-blue-500 focus:border-blue-500" />
